...
function customHook() {
  ...
  const showTooltip = function () {
    ...
    tooltip.style.top = `${cr[0].top - tooltip.offsetHeight}px`;
    tooltip.style.left = `${cr[0].left}px`;
  };
  ...
  document.addEventListener("selectionchange", () => {
    const selection = document.getSelection();
    cr = window.getSelection() && window.getSelection().rangeCount ? window.getSelection().getRangeAt(0).getClientRects() : [];
  ...
  document.addEventListener("mouseover", ev => {
    ...
    for (const element of cr) {
      hideTooltip();
      if (ev.pageX >= element.left && ev.pageX <= element.right && ev.pageY >= element.top && ev.pageY <= element.bottom) {
        showTooltip();
      }
      break;
    }
